...
function DevApp() {
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-surface-deep">
      {/* Dev Mode Tab Bar */}
      <div
        className="fixed top-0 left-0 right-0 z-50 flex items-center gap-1 px-4 py-2"
        style={{
      ...
      >
        <span
          className="text-[11px] text-gold-400 font-semibold tracking-widest uppercase mr-4"
        ...
        {tabs.map((tab) => (
          <button
            ...
            style={{
              background:
                activeTab === tab.id ? "rgba(232,185,49,0.12)" : "transparent",
              color: activeTab === tab.id ? "#e8b931" : "#8e8e99",
              border:
                activeTab === tab.id
                  ? "1px solid rgba(232,185,49,0.2)"
      ...
      <div className="pt-12 w-full">
        {activeTab === "dashboard" && (
          <Dashboard onNavigate={(p) => setActiveTab(p as Tab)} />
        )}
        {activeTab === "popup" && (
        ...
        {activeTab === "settings" && (
          <Settings onNavigate={(p) => setActiveTab(p as Tab)} />
        )}
      </div>
    </div>
  );
